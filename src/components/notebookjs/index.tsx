...
const defaultMenuItems: any[] = [EditProps as any];
const defaultEditors: any[] = [PropertiesEditor as any];
...
export default function NotebookJS({
  ...
  const [focusRequest, setFocusRequest] = useState<{ id: string; where: "start" | "end" } | null>(null);
  const dispatchNotebookBlocks = async (action: any) => {
    ...
    if (action.type === "focus-move") {
      ...
      if (idx !== -1) {
        ...
        else if (action.dir === "down" && idx < ids.length - 1) targetId = ids[idx + 1];
        if (targetId) setFocusRequest({ id: targetId, where: action.dir === "up" ? "end" : "start" });
      }
    }
  };
  ...
  return (
    <>
      <NotebookProvider value={{ blocks: blockTypes as any, tools: tools as any, editors: editors as any, menuItems: menuItems as any, dispatcher: dispatchNotebookBlocks, editorFns: editorFns as any, refsMap: refsMap.current }}>
        ...
        <div className="blocks-container w-full">
          {notebookBlocks.length === 0 ? (
            <div className="flex justify-center items-center py-4">
              ...
              >
                Create {(blockTypes as any[])[0].label || (blockTypes as any[])[0].name || "block"} block
              </Button>
            </div>
          ) : (
